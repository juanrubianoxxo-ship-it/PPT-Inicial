...
st.set_page_config(page_title='Presentación Inicial | Streamlit', page_icon='📊', layout='wide', initial_sidebar_state='expanded')

st.markdown('''
<style>
    :root { --brand-red: #d71920; --ink: #202124; }
    .block-container { padding-top: 2rem; padding-bottom: 3rem; max-width: 1400px; }
    h1, h2, h3 { color: var(--ink); }
    [data-testid="stSidebar"] { background: linear-gradient(180deg, #fff 0%, #f7f7f8 100%); border-right: 1px solid #e7e7e7; }
    [data-testid="stSidebar"] h2 { color: var(--brand-red); }
    div[data-testid="stExpander"] { border: 1px solid #e2e4e8; border-radius: 12px; margin-bottom: .65rem; background: #fff; }
    .hero { padding: 1.45rem 1.7rem; border-radius: 16px; background: linear-gradient(135deg, #d71920 0%, #9e1117 100%); color: #fff; margin-bottom: 1.3rem; box-shadow: 0 8px 25px rgba(90, 8, 12, .16); }
    .hero h1 { color: #fff; margin: 0 0 .35rem 0; font-size: 2.15rem; }
    .hero p { color: #fff; opacity: .92; margin: 0; font-size: 1rem; }
</style>
<div class="hero">
  <h1>Presentación Inicial</h1>
  <p>Construye, guarda y genera la presentación de un nuevo punto comercial.</p>
</div>
''', unsafe_allow_html=True)
st.caption('Completa las secciones en orden. Los datos de Book.xlsx se cargan automáticamente y el JSON permite guardar o restaurar el trabajo.')
